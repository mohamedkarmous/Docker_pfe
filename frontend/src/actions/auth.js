...
export const loadUser = () => async (dispatch) => {
  ...
  try {
    const res = await axios.post(url + "/api/account/token", {
...
export const register = ({
  ...
  try {
    const res = await axios.post(url + "/api/register", body, config);
...
export const login = (username, password) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(url + "/api/login", body, config);
    dispatch({
      type: LOGIN_SUCCESS,
      payload: res.data,
    });
